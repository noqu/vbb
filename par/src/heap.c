...
/*^*/
/*------------------------ H_GET_FROM_HEAP ----------------------*/
/* Holt den Knoten mit der groessten Prioritaet vom Heap.
 * Wenn Elemente in der LIFO-Queue sind, wird das erste davon
 * zurueckgegeben, anderenfalls wird der Knoten mit der kleinsten
 * Prioritaet vom Binaerbaum zurueckgegeben. Rueckgabewert ist
...
/*^*/
/*------------------------ H_INIT_HEAP --------------------------*/
/* Initialisiert den Binaerbaum und den LIFO-Stack
 * (Wird nur vom Dispatcher verwendet, nicht von anderen Prozessen)
 */
...
public	int	h_new_incumbent (new, to_mem)
	...
	if (akt->l_bound >= incumbent->value)
	   {
	    ...
	    sts = q_write_bl (to_mem, Q_EMPTY_NODE, elim, &m_finished);
	    R_ST
...
/*^*/
/*------------------------ H_PUT_ON_HEAP ------------------------*/
/* Legt den Knoten 'node' auf den Heap. Wenn die kritische
 * Speicherplatzgrenze bereits erreicht ist (heap_full), wird der
 * Knoten auf den LIFO-Stack gelegt, ansonsten wird er in den
 * Binaerbaum einsortiert.
 * (Wird nur vom Dispatcher verwendet, nicht von anderen Prozessen)
 */
