...
/*------------------------ KL_INPUT ----------------------------*/
/* Liest eine Kantenliste von der Datei 'idat'. Daraus wird ein
 * Ueberdeckungsproblem konstruiert, indem die Mengen pred und succ
 * fuer jede Server-Ecke (Quelle) und Client-Ecke (Senke) angelegt
 * werden. Xsize gibt die Menge der Senken, Ysize die Menge der Quellen
 * an. In dem Integer-Array 'values' stehen die Werte fuer die
 * Quellen.
 * Quellen und Senken muessen jeweils von 0 an lueckenlos
 * durchnumeriert sein. In der Datei stehen zunaechst die beiden
 * Parameter:
 *     QUELLEN=<anzahl>
 *     SENKEN=<anzahl>
 * Darauf folgen die Server-Ecken (Quellen) mit den Bewertungen 
 * in der Form:
 *     <eckennr> ; <wert>
 * Die Client-Ecken (Senken) werden nicht aufgezaehlt, sondern implizit
 * durch ihre Anzahl beschrieben. Als letztes folgt eine Liste der
 * Kanten in der Form:
 *     <quelle> <senke>
 * Kommentare sind nach Shell-Konventionen moeglich.
 * Es muss jede Client-Ecke von mindestens einem Server erreichbar
 * sein. Fehler werden auf der Standardausgabe gemeldet und liefern
 * einen Rueckgabestatus von READ_ERR. Bei fehlerfreier Bearbeitung
 * liefert die Funktion OK_TRUE.
 */
